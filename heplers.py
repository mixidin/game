...
def display_inventory():
    ...
    elif 'Костяной меч' in player['inventory']:
        might = input('''Желаешь применить меч?
    1 - да
    2 - нет
    ''')
        if might == '1':
            player['attack'] += 12
            print(f'Готово! Теперь твой урон от атаки равен {player["attack"]}')
            player['inventory'].remove('Костяной меч')
...
def earn():
    print('Добро пожаловать на завод! У тебя есть 40% шанс заработать 500 монет. Соответственно, 60% чтобы их потерять')
